/**
 * Manus Search Adapter — Search for Paragon (Manus REST backend).
 *
 * Delegates to the Manus REST API (/api/data) for full-text search
 * and browsing. Falls back to local IndexedDB search for instant
 * offline results.
 */
import type { SearchAdapter, SearchResult, SearchFilters } from '@/adapters/types';
import { getDatabaseAdapter } from '@/adapters/registry';
import { searchItemsLocally } from '@/lib/offlineStore';

/**
 * Map a raw API row to SearchResult.
 */
...
export class ManusSearchAdapter implements SearchAdapter {
  ...
  async search(
    ...
    if (ftsError) {
      console.warn('[ManusSearchAdapter] FTS search error:', ftsError.message);
    }
    ...
    if (fuzzyError) {
      console.warn('[ManusSearchAdapter] Fuzzy search error:', fuzzyError.message);
      return [];
    }
  ...
  async browse(filters: SearchFilters, maxResults = 20): Promise<SearchResult[]> {
    ...
    if (filters.tagId) {
      // For tag filter, get item IDs from item_tags first
    ...
    if (error) {
      console.warn('[ManusSearchAdapter] Browse filtered items error:', error.message);
      return [];
    }
  ...
  clearCache(): void {
    // No-op — the LRU cache lives in serverSearch.ts
  }
}
