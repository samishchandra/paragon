...
export async function generateTestData(options: GenerateOptions): Promise<GenerateResult> {
  ...
  if (clearExisting) {
    ...
    const userItemIds = (userItems || []).map((i: any) => i.id);
  ...
  const tagInserts = TEST_TAGS.map((t: any) => ({
    name: t.name,
    color: t.color,
    user_id: userId,
  }));
  ...
  (createdTags || []).forEach((t: any) => tagNameToId.set(t.name, t.id));

  // Step 3: Create lists
  onProgress?.(25, 'Creating lists...');
  const listInserts = TEST_LISTS.map((l: any, i: number) => ({
    name: l.name,
    icon: l.icon,
    color: l.color,
    type: l.type,
  ...
  const taskLists = (createdLists || []).filter((l: any) => l.type === 'task');
  const noteLists = (createdLists || []).filter((l: any) => l.type === 'note');
  ...
  for (let i = 0; i < taskItems.length; i += batchSize) {
    ...
    insertedTaskIds.push(...(data || []).map((d: any) => d.id));
  ...
  for (let i = 0; i < noteItems.length; i += batchSize) {
    ...
    insertedNoteIds.push(...(data || []).map((d: any) => d.id));
...
export async function clearAllData(userId: string): Promise<{ items: number; lists: number; tags: number }> {
  ...
  const { count: itemCount } = await apiQuery({ table: 'items', select: '*', filters: { user_id: userId }, count: 'exact' });
  const { count: listCount } = await apiQuery({ table: 'lists', select: '*', filters: { user_id: userId }, count: 'exact' });
  const { count: tagCount } = await apiQuery({ table: 'tags', select: '*', filters: { user_id: userId }, count: 'exact' });
