...
let _savedFolderName: string = '';  // Name from IndexedDB even if not yet reconnected
...
/**
 * Returns the name of a previously saved folder (from IndexedDB) even if
 * permission hasn't been re-granted yet. Useful for showing a "Reconnect" UI.
 */
export function getSavedFolderName(): string {
  return _savedFolderName || '';
}

/**
 * Returns true if there's a saved handle in IndexedDB that hasn't been
 * reconnected yet (needs user gesture to re-grant permission).
 */
export function hasSavedFolder(): boolean {
  return !_dirHandle && _savedFolderName !== '';
}
 ...
 * First tries queryPermission (no user gesture needed) to check if
 * permission is already granted. If not, stores the folder name so
 * the UI can show a "Reconnect" button.
 * Returns true if the handle was fully restored with permission.
 */
export async function restoreFolder(): Promise<boolean> {
  const handle = await loadHandle();
  if (!handle) return false;

  // Always remember the folder name for the UI
  _savedFolderName = handle.name;

  try {
    // First try queryPermission — does NOT require a user gesture
    const queryPerm = await (handle as any).queryPermission({ mode: 'readwrite' });
    ...
    // queryPermission not supported or handle is stale
  }

  // Permission not yet granted — UI should show "Reconnect" button
  return false;
}

/**
 * Re-grant permission to a previously saved folder handle.
