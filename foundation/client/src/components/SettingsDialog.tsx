...
import { getAuthAdapter, getAIAdapter, getBackupAdapter, getThemeConfig } from '@/adapters/registry';
...
const ALL_SECTIONS: { id: SettingsSection; label: string; icon: React.ElementType }[] = [
...
/** Filter sections based on which adapters are active */
function getVisibleSections() {
  const hidden = new Set<SettingsSection>();
  try {
    if (!getAuthAdapter().requiresAuth) hidden.add('account');
  } catch { hidden.add('account'); }
  try {
    if (!getBackupAdapter().isEnabled()) hidden.add('backup');
  } catch { hidden.add('backup'); }
  return ALL_SECTIONS.filter(s => !hidden.has(s.id));
}
...
export function SettingsDialog({ open, onOpenChange, initialSection }: SettingsDialogProps) {
  const [activeSection, setActiveSection] = useState<SettingsSection>(SECTIONS[0]?.id || 'general');
        ...
        <div className="flex h-[65vh] max-h-[65vh]">
          ...
          <nav className={cn(
            ...
            <div className="px-5 py-3 border-t border-border/50 shrink-0 flex items-center gap-2">
              <img src={getThemeConfig().appIconSmallUrl || getThemeConfig().appIconUrl} alt={getThemeConfig().appName} className="w-4 h-4" />
              <p className="text-xs text-muted-foreground">{getThemeConfig().appName}</p>
            </div>
          </nav>
