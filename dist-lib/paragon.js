  ...
  return /* @__PURE__ */ f(
    Vg,
    {
      ...
      components: {
        ...
        ) : /* @__PURE__ */ f(Ef, { className: ue("size-4", l), ...u }),
        DayButton: _y,
...
Bd.displayName = y0;
var v0 = "MenuSub", [iC, Fd] = gn(v0), pr = "MenuSubTrigger", Ud = S.forwardRef(
  (e, t) => {
...
`);
  ...
  render() {
    if (this.state.hasError) {
      ...
      return /* @__PURE__ */ f("div", { className: ue("flex-1 flex items-center justify-center p-6", this.props.className), children: /* @__PURE__ */ R("div", { className: "flex flex-col items-center max-w-md w-full text-center gap-4", children: [
...
const p1 = () => {
  ...
  const [ae] = K(() => p1()), [be, Ve] = K(C), [ve, or] = K(""), qe = V(C), kt = V(""), Ct = V(null), [sr, Qa] = K(0), Ur = !!(ft && ft.length > 0 && xn), { state: Ze, executeAction: Yr, abort: ku, reset: Bt } = t1(xn), [ts, ns] = K(null), [Cu, Mu] = K({ selectionTop: 0, selectionBottom: 0, selectionCenterX: 0 }), Su = V(xn);
  Su.current = xn;
  const Ja = V(le);
  Ja.current = le;
      ...
      if (W === "markdown" && qe.current === "wysiwyg") {
        ...
        he.forEach((ie) => {
          ...
          fe = fe.replace(se, (we, de) => {
            const Ee = F.parse(de.trim(), { async: !1 });
          ...
          const de = se.split("|").map((Tt) => Tt.trim());
          let Ee = "", ze = "left", Le = null;
          de.length === 1 ? Ee = de[0] : de.length === 2 ? (Ee = de[0], /^\d+$/.test(de[1]) ? Le = de[1] : ["left", "center", "right"].includes(de[1]) ? ze = de[1] : Ee = se) : de.length === 3 ? (Ee = de[0], ["left", "center", "right"].includes(de[1]) && (ze = de[1]), /^\d+$/.test(de[2]) && (Le = de[2])) : Ee = se;
          const De = Le ? ` width="${Le}" style="width: ${Le}px"` : "", jt = ` data-align="${ze}"`;
          return `<img src="${we.trim()}" alt="${Ee}"${jt}${De} />`;
        ...
        let Te = F.parse(fe, { async: !1 });
        Te = f1(Te), Te = m1(Te), Te = Te.replace(/(<table>[\s\S]*<\/table>)\s*<!--\s*header-column\s*-->/gi, (ie, se) => se.replace(/(<tbody>[\s\S]*?<\/tbody>)/gi, (we) => we.replace(/<tr>([\s\S]*?)<\/tr>/gi, (de, Ee) => `<tr>${Ee.replace(/^([\s\S]*?)<td>([\s\S]*?)<\/td>/i, "$1<th>$2</th>")}</tr>`)));
        const ce = (ie) => {
          let se = ie;
          ...
          const se = ie.match(/^( *)/), we = se ? se[1].length : 0, de = Math.floor(we / 2), Ee = ie.trimStart(), ze = Ee.match(/^-\s*\[(x| )\]\s*(.*)$/);
          if (ze)
          ...
          }, we = Math.min(...ie.map((Ee) => Ee.depth));
          return se(0, we).html;
        };
        Te = Te.replace(
          /(<t[dh][^>]*>)([\s\S]*?)(<\/t[dh]>)/gi,
          (ie, se, we, de) => {
            const Ee = /<img\s/i.test(we), ze = /<br\s*\/?>/i.test(we), Le = /(?:^|<br\s*\/?>)\s*(?:- |\d+\. )/i.test(we);
            if (!Ee && !ze && !Le) return ie;
            let De = we.trim();
            De = De.replace(/^<p>([\s\S]*)<\/p>$/i, "$1").trim();
            const jt = De.split(/<br\s*\/?>/i).filter((Dt) => Dt.trim());
            if (jt.length <= 1 && !Le)
              return Ee ? `${se}${Yt(De)}${de}` : ie;
            const Tt = [];
            let Kt = [];
            const cr = () => {
            ...
            return cr(), `${se}${Tt.join("")}${de}`;
          }
        ), queueMicrotask(() => {
          _.isDestroyed || _.commands.setContent(Te);
        });
      }
      ...
      if (!(!(F.metaKey || F.ctrlKey) && he !== " ")) {
        ...
        if (!ae && (F.metaKey || F.ctrlKey) && F.key === "f") {
          if (F.preventDefault(), _) {
            const { state: ke } = _, { from: Te, to: ce } = ke.selection;
            if (Te !== ce) {
              const Se = ke.doc.textBetween(Te, ce, " ");
              Se.trim() && Hu(Se.trim());
            }
          }
          Ft(!0), fs((ke) => ke + 1);
        ...
        if (F.key === " ")
          try {
            const { state: ke } = _, { selection: Te } = ke, { $from: ce } = Te, Se = ce.nodeBefore?.textContent || "";
            if (Se === "#####") {
  ...
  const ai = Y((W, F) => {
    ...
    if (F) {
      ...
      const { from: fe, to: ke } = _.state.selection, Te = _.view.coordsAtPos(fe), ce = _.view.coordsAtPos(ke);
      he = { top: ce.bottom + 8, left: (Te.left + ce.left) / 2 };
    }
    ...
    if (!he) return;
    const { from: fe, to: ke } = _.state.selection, Te = fe !== ke ? _.state.doc.textBetween(fe, ke, `
`) : "", ce = he.scope === "document" || !Te ? _.getText() : Te, Se = _.view.coordsAtPos(fe), Yt = _.view.coordsAtPos(ke);
    Mu({
      selectionTop: Se.top,
