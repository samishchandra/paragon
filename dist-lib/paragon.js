...
const mh = (e) => ({
  ...
  async fn(t) {
    ...
    const k = M / 2 - w / 2, N = S / 2 - m[g] / 2 - 1, C = Lt(u[v], N), D = Lt(u[b], N), L = C, P = S - m[g] - D, O = S / 2 - m[g] / 2 + k, $ = bs(L, O, P), W = !l.arrow && Cn(o) != null && O !== $ && s.reference[g] / 2 - (O < L ? C : D) - m[g] / 2 < 0, V = W ? O < L ? O - L : O - P : 0;
    return {
      [p]: f[p] + V,
      data: {
        [p]: $,
...
var ca = "PopperContent", [ym, vm] = ec(ca), sc = T.forwardRef(
  (e, t) => {
    const {
      ...
      onPlaced: g,
      ...m
    } = e, y = tc(ca, n), [v, b] = T.useState(null), x = Ie(t, (_) => b(_)), [M, w] = T.useState(null), E = mm(M), S = E?.width ?? 0, k = E?.height ?? 0, N = r + (s !== "center" ? "-" + s : ""), C = typeof d == "number" ? d : { top: 0, right: 0, bottom: 0, left: 0, ...d }, D = Array.isArray(c) ? c : [c], L = D.length > 0, P = {
      padding: C,
...
var Ke = "MenuContent", [Pg, pa] = Kt(Ke), Pc = T.forwardRef(
  ...
  (e, t) => {
    const {
      ...
      ...m
    } = e, y = Gt(Ke, n), v = rr(Ke, n), b = Co(n), x = Lc(n), M = Dg(n), [w, E] = T.useState(null), S = T.useRef(null), k = Ie(t, S, y.onContentChange), N = T.useRef(0), C = T.useRef(""), D = T.useRef(0), L = T.useRef(null), P = T.useRef("right"), O = T.useRef(0), $ = g ? Sc : T.Fragment, W = g ? { as: $g, allowPinchZoom: !0 } : void 0, V = (A) => {
                    ...
                    children: /* @__PURE__ */ h(
                      uc,
                      {
                        ...
                        onKeyDown: le(m.onKeyDown, (A) => {
                          ...
                          Dc.includes(A.key) && J.reverse(), Kg(J);
                        }),
...
const Ee = (e) => {
  ...
  const e = Ee("color"), t = Ee("font"), n = Ee("text"), r = Ee("font-weight"), o = Ee("tracking"), s = Ee("leading"), i = Ee("breakpoint"), a = Ee("container"), l = Ee("spacing"), c = Ee("radius"), d = Ee("shadow"), u = Ee("inset-shadow"), f = Ee("text-shadow"), p = Ee("drop-shadow"), g = Ee("blur"), m = Ee("perspective"), y = Ee("aspect"), v = Ee("ease"), b = Ee("animate"), x = () => ["auto", "avoid", "all", "avoid-page", "page", "left", "right", "column"], M = () => [
    ...
    span: ["full", Ct, Q, Z]
  }, Ct, Q, Z], L = () => [Ct, "auto", Q, Z], P = () => ["auto", "min", "max", "fr", Q, Z], O = () => ["start", "end", "center", "between", "around", "evenly", "stretch", "baseline", "center-safe", "end-safe"], $ = () => ["start", "end", "center", "stretch", "center-safe", "end-safe"], W = () => ["auto", ...k()], V = () => [rn, "auto", "full", "dvw", "dvh", "lvw", "lvh", "svw", "svh", "min", "max", "fit", ...k()], R = () => [e, Q, Z], A = () => [...M(), Di, Si, {
    position: [Q, Z]
  }], H = () => ["no-repeat", {
    ...
    Q,
    Z
  ], K = () => ["", ce, Rn, Ht], B = () => ["solid", "dashed", "dotted", "double"], J = () => ["normal", "multiply", "screen", "overlay", "darken", "lighten", "color-dodge", "color-burn", "hard-light", "soft-light", "difference", "exclusion", "hue", "saturation", "color", "luminosity"], _ = () => [ce, es, Di, Si], ee = () => [
    // Deprecated since Tailwind CSS v4.0.0
    "",
    "none",
    g,
...
function vx(e, t, n, r) {
  const { addDays: o, endOfBroadcastWeek: s, endOfISOWeek: i, endOfMonth: a, endOfWeek: l, getISOWeek: c, getWeek: d, startOfBroadcastWeek: u, startOfISOWeek: f, startOfWeek: p } = r, g = e.reduce((m, y) => {
    ...
    const w = x.reduce((S, k) => {
      const N = n.ISOWeek ? c(k) : d(k), C = S.find((L) => L.weekNumber === N), D = new Ud(k, y, r);
