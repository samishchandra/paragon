  ...
  const ft = U(async (B) => {
    if (O) {
      if (B === "markdown" && qe.current === "wysiwyg") {
        ...
        Te = Te.replace(/<!--\s*list-break\s*-->/g, ""), Te = px(Te), Te = gx(Te), Te = Te.replace(/(<table>[\s\S]*<\/table>)\s*<!--\s*header-column\s*-->/gi, (ae, se) => se.replace(/(<tbody>[\s\S]*?<\/tbody>)/gi, (Ne) => Ne.replace(/<tr>([\s\S]*?)<\/tr>/gi, (de, Me) => `<tr>${Me.replace(/^([\s\S]*?)<td>([\s\S]*?)<\/td>/i, "$1<th>$2</th>")}</tr>`)));
        const le = (ae) => {
          let se = ae;
