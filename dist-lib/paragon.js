...
function Zt({
  ...
  return /* @__PURE__ */ m(
    ...
    {
      "data-slot": "button",
      className: me(Ks({ variant: t, size: n, className: e })),
      ...r
    },
    void 0,
    !1,
...
function $b({
  ...
  return /* @__PURE__ */ m(
    qg,
    {
      ...
      classNames: {
        root: me("w-fit", l.root),
        months: me(
          "flex gap-4 flex-col md:flex-row relative",
          l.months
        ),
        month: me("flex flex-col w-full gap-4", l.month),
        nav: me(
        ...
        weekdays: me("flex", l.weekdays),
        weekday: me(
          "text-muted-foreground rounded-md flex-1 font-normal text-[0.8rem] select-none",
          l.weekday
        ),
        week: me("flex w-full mt-2", l.week),
        week_number_header: me(
        ...
        range_middle: me("rounded-none", l.range_middle),
        range_end: me("rounded-r-md bg-accent", l.range_end),
        today: me(
      ...
      components: {
        ...
        Chevron: ({ className: c, orientation: u, ...d }) => u === "left" ? /* @__PURE__ */ m(xm, { className: me("size-4", c), ...d }, void 0, !1, {
          fileName: "/home/ubuntu/manus-markdown-editor/client/src/components/ui/calendar.tsx",
          lineNumber: 139,
          columnNumber: 15
          ...
          this
        ) : /* @__PURE__ */ m(Em, { className: me("size-4", c), ...d }, void 0, !1, {
          fileName: "/home/ubuntu/manus-markdown-editor/client/src/components/ui/calendar.tsx",
          lineNumber: 153,
          columnNumber: 13
        }, this),
...
function Fb({
  ...
  return /* @__PURE__ */ m(
    ...
    {
      ref: s,
      className: me("date-picker-portal", t === "dark" ? "dark" : ""),
      "data-theme": t,
      style: {
        position: "fixed",
        top: n.top,
...
Fu.displayName = w0;
var N0 = "MenuSub", [d1, zu] = gn(N0), po = "MenuSubTrigger", Uu = E.forwardRef(
  (e, t) => {
...
function Is({
  ...
  return /* @__PURE__ */ m(
    py,
    {
      ...
      className: me("bg-border -mx-1 my-1 h-px", e),
      ...t
    },
    void 0,
    !1,
...
`);
  ...
  render() {
    if (this.state.hasError) {
      ...
      return /* @__PURE__ */ m("div", { className: me("flex-1 flex items-center justify-center p-6", this.props.className), children: /* @__PURE__ */ m("div", { className: "flex flex-col items-center max-w-md w-full text-center gap-4", children: [
...
const bx = () => {
  ...
  const [ie] = K(() => bx()), [we, Ve] = K(x), [ve, ro] = K(""), qe = V(x), kt = V(""), xt = V(null), [so, Qi] = K(0), Yo = !!(mt && mt.length > 0 && yn), { state: Ze, executeAction: jo, abort: kd, reset: Ft } = ox(yn), [ts, ns] = K(null), [xd, Cd] = K({ selectionTop: 0, selectionBottom: 0, selectionCenterX: 0 }), Ed = V(yn);
  Ed.current = yn;
  const Ji = V(ce);
  Ji.current = ce;
  ...
  const O = jd({
    ...
    onSelectionUpdate: ({ editor: B }) => {
      if (I) {
        const { from: F, to: pe, empty: ue } = B.state.selection;
        I({ from: F, to: pe, empty: ue });
      }
    }
  });
  Q(() => () => {
    ...
    if (O) {
      if (B === "markdown" && qe.current === "wysiwyg") {
        ...
        ue = hx(ue), pe.forEach((ae) => {
          ...
          const de = se.split("|").map((Mt) => Mt.trim());
          let Me = "", We = "left", Re = null;
          de.length === 1 ? Me = de[0] : de.length === 2 ? (Me = de[0], /^\d+$/.test(de[1]) ? Re = de[1] : ["left", "center", "right"].includes(de[1]) ? We = de[1] : Me = se) : de.length === 3 ? (Me = de[0], ["left", "center", "right"].includes(de[1]) && (We = de[1]), /^\d+$/.test(de[2]) && (Re = de[2])) : Me = se;
          const Se = Re ? ` width="${Re}" style="width: ${Re}px"` : "", jt = ` data-align="${We}"`;
          return `<img src="${Ne.trim()}" alt="${Me}"${jt}${Se} />`;
        }), ue = ue.replace(new RegExp("(?<!`)==((?:(?!==)[^\\n])+)==(?!`)", "g"), "<mark>$1</mark>"), ue = ue.replace(/@([^@\n]+)@/g, (ae, se) => {
          const Ne = cn(se);
          if (Ne) {
            const de = Pi(Ne);
            return `<span data-type="date-pill" data-date="${Ne}" class="date-pill ${de}"><span class="date-icon">📅</span><span class="date-text">${se.trim()}</span></span>`;
          }
          return ae;
        }), zo && !_.tagPills && (ue = ue.replace(new RegExp("(?:^|(?<=\\s))#([a-zA-Z][a-zA-Z0-9_-]*|[a-zA-Z0-9_-]*[a-zA-Z][a-zA-Z0-9_-]*)(?=\\s|$|[.,;:!?)\\]])", "gm"), (ae, se) => {
          const Ne = wo(se);
        ...
        ), Te = px(Te), Te = gx(Te), Te = Te.replace(/(<table>[\s\S]*<\/table>)\s*<!--\s*header-column\s*-->/gi, (ae, se) => se.replace(/(<tbody>[\s\S]*?<\/tbody>)/gi, (Ne) => Ne.replace(/<tr>([\s\S]*?)<\/tr>/gi, (de, Me) => `<tr>${Me.replace(/^([\s\S]*?)<td>([\s\S]*?)<\/td>/i, "$1<th>$2</th>")}</tr>`)));
        const le = (ae) => {
          let se = ae;
          ...
          if (/<img\s/i.test(ae)) {
            const se = /(<img\s[^>]*\/?>)/gi;
            return ae.split(se).filter((de) => de.trim()).map((de) => /^<img\s/i.test(de) ? Ee(de) : de.trim() ? `<p>${le(de.trim())}</p>` : "").join("");
          }
          if (/^!\[/.test(ae)) {
            const se = ae.match(/^!\[([^\]]*)\]\(([^)]+)\)$/);
            if (se)
          ...
          const se = ae.match(/^( *)/), Ne = se ? se[1].length : 0, de = Math.floor(Ne / 2), Me = ae.trimStart(), We = Me.match(/^-\s*\[(x| )\]\s*(.*)$/);
          if (We)
            return { type: "task", depth: de, text: We[2].trim(), checked: We[1] === "x" };
          const Re = Me.match(/^-\s+(.+)$/);
          if (Re)
            return { type: "ul", depth: de, text: Re[1].trim() };
          const Se = Me.match(/^(\d+)\.\s+(.+)$/);
          return Se ? { type: "ol", depth: de, text: Se[2].trim(), index: parseInt(Se[1], 10) } : null;
        }, Go = (ae) => {
          ...
          }, Ne = Math.min(...ae.map((Me) => Me.depth));
          return se(0, Ne).html;
        };
        Te = Te.replace(
          /(<t[dh][^>]*>)([\s\S]*?)(<\/t[dh]>)/gi,
          (ae, se, Ne, de) => {
            const Me = /<img\s/i.test(Ne), We = /<br\s*\/?>/i.test(Ne), Re = /(?:^|<br\s*\/?>)\s*(?:- |\d+\. )/i.test(Ne);
            if (!Me && !We && !Re) return ae;
            let Se = Ne.trim();
            Se = Se.replace(/^<p>([\s\S]*)<\/p>$/i, "$1").trim();
            const jt = Se.split(/<br\s*\/?>/i).filter((St) => St.trim());
            if (jt.length <= 1 && !Re)
              return Me ? `${se}${Yt(Se)}${de}` : ae;
            const Mt = [];
            let Kt = [];
            const lo = () => {
            ...
            return lo(), `${se}${Mt.join("")}${de}`;
          }
        ), queueMicrotask(() => {
          O.isDestroyed || O.commands.setContent(Te);
        });
      }
    ...
    getTableOfContents: () => {
      ...
      return O.state.doc.descendants((F, pe) => {
        if (F.type.name === "heading") {
          const ue = F.attrs.level, ke = F.textContent.trim();
          ke && B.push({ id: `toc-heading-${pe}`, text: ke, level: ue, pos: pe });
        }
      }), B;
    },
      ...
      if (!(!O || O.isDestroyed))
        try {
          ...
          if (pe instanceof HTMLElement) {
            const ue = O.view.dom.closest(".editor-content-wrapper");
            if (ue) {
              const ke = ue.getBoundingClientRect(), le = pe.getBoundingClientRect().top - ke.top + ue.scrollTop;
              ue.scrollTo({ top: le - 20, behavior: "smooth" });
            } else
      ...
      if (!(!(F.metaKey || F.ctrlKey) && pe !== " ")) {
        ...
        if (!ie && (F.metaKey || F.ctrlKey) && F.key === "f") {
          if (F.preventDefault(), O) {
            const { state: ke } = O, { from: Te, to: le } = ke.selection;
            if (Te !== le) {
              const Ee = ke.doc.textBetween(Te, le, " ");
              Ee.trim() && Bd(Ee.trim());
            }
          }
          zt(!0), ms((ke) => ke + 1);
        ...
        if (F.key === " ")
          try {
            const { state: ke } = O, { selection: Te } = ke, { $from: le } = Te, Ee = le.nodeBefore?.textContent || "";
            if (Ee === "#####") {
            ...
            if (Yt) {
              ...
              if (Go && ae) {
                const se = ke.tr, Ne = le.pos - Ee.length, de = le.pos;
                se.delete(Ne, de);
  ...
  const ia = U((B, F) => {
    ...
    if (F) {
      ...
      pe = { top: ue.bottom + 4, left: ue.left };
    } else {
      const { from: ue, to: ke } = O.state.selection, Te = O.view.coordsAtPos(ue), le = O.view.coordsAtPos(ke);
      pe = { top: le.bottom + 8, left: (Te.left + le.left) / 2 };
    }
    ...
    if (!pe) return;
    const { from: ue, to: ke } = O.state.selection, Te = ue !== ke ? O.state.doc.textBetween(ue, ke, `
`) : "", le = pe.scope === "document" || !Te ? O.getText() : Te, Ee = O.view.coordsAtPos(ue), Yt = O.view.coordsAtPos(ke);
    Cd({
      selectionTop: Ee.top,
      selectionBottom: Yt.bottom,
      selectionCenterX: (Ee.left + Yt.right) / 2
    }), ns(null), jo(B, pe.label, le, { from: ue, to: ke }, F);
  }, [O, mt, jo]), Wd = U(() => {
...
` + F).run(), Ft();
    ...
    if (!(Ze.status !== "complete" && Ze.status !== "error"))
      if (Ze.status === "complete") {
        const { action: B, actionLabel: F, inputText: pe, selectionRange: ue } = Ze;
        Ft(), jo(B, F, pe, ue);
      } else
        Ft();
  }, [Ze, Ft, jo]);
