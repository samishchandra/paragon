  ...
  const mt = Y(async (W) => {
    if (_) {
      if (W === "markdown" && qe.current === "wysiwyg") {
        ...
        Ee = Ee.replace(/<!--\s*list-break\s*-->/g, ""), Ee = m1(Ee), Ee = p1(Ee), Ee = Ee.replace(/(<table>[\s\S]*<\/table>)\s*<!--\s*header-column\s*-->/gi, (ie, se) => se.replace(/(<tbody>[\s\S]*?<\/tbody>)/gi, (we) => we.replace(/<tr>([\s\S]*?)<\/tr>/gi, (ue, Te) => `<tr>${Te.replace(/^([\s\S]*?)<td>([\s\S]*?)<\/td>/i, "$1<th>$2</th>")}</tr>`)));
        const ce = (ie) => {
          let se = ie;
